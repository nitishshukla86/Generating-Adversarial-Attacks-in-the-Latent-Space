...
def train_discriminator(real_images,labels,target=None):
    #D.train()
    opt_d.zero_grad()
    ...
    if target >-1:
        ...
        fake_loss = F.cross_entropy(fake_preds, 9-labels)
...
def train_generator(real_imgs,labels,target):
    ...
    if target > -1:
        ...
        loss = 0.1*F.cross_entropy(preds, targets)  +0.9*F.l1_loss(real_imgs,fake_images)
    else:
        loss = 0.1*F.cross_entropy(preds, 9-labels)  +0.9*torch.abs(real_imgs-fake_images).sum()
...
def fit(epochs, lr, target,trainloader,start_idx=1):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            num_eq_to_target=(torch.argmax(D(G(imgs)),dim=1)==target).sum().item() if target >-1 else 0
...
def test(loader,target,ds='mnist'):
...
if __name__=='__main__':
    ...
    parser.add_argument('--t', type=int, default=-1,help='target class (default -1 :untargeted)')
